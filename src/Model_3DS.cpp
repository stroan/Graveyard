...
Model_3DS::Object* Model_3DS::GetObjectByName(const std::string& name) {
  for (int i = 0; i < numObjects; i++) {
    if (name == Objects[i].name) { return &Objects[i]; }
  }
  return NULL;
}
...
void Model_3DS::Load(char *name)
{
  ...
  fprintf(stderr, "after \n");
	...
	for (int j = 0; j < numMaterials; j++)
	{
      ...
      Materials[j].tex = Texture::ColorTexture(r, g, b);
			Materials[j].textured = true;
		}
	}

  // Create VBOs and IBOs for all objects
  for (int i = 0; i < numObjects; i++) {
    
    float maxX, minX, maxY, minY, maxZ, minZ;
    maxX = minX = Objects[i].Vertexes[0];
    maxY = minY = Objects[i].Vertexes[1];
    maxZ = minZ = Objects[i].Vertexes[2];

    for (int j = 1; j < Objects[i].numVerts; j++) {
      float x = Objects[i].Vertexes[j * 3 + 0];
      float y = Objects[i].Vertexes[j * 3 + 1];
      float z = Objects[i].Vertexes[j * 3 + 2];
    ...
    Objects[i].pos.x = centerX;
    Objects[i].pos.y = centerY;
    Objects[i].pos.z = centerZ;

    float* currentVert = Objects[i].Vertexes;
    float* currentNorm = Objects[i].Normals;
    float* currentTex = Objects[i].TexCoords;

    std::vector<VertPosTexNorm> v;
    for (int j = 0; j < Objects[i].numVerts; j++) {
      currentVert[0] -= centerX;
      currentVert[1] -= centerY;
      currentVert[2] -= centerZ;
      v.push_back(VertPosTexNorm(currentVert,currentTex,currentNorm));
      currentVert += 3;
      currentNorm += 3;
      currentTex += 2;
    }
    ...
    unsigned short* indexes = new unsigned short[totalSubFaces * 3];
    unsigned short* currentIndex = indexes;
    int indexOffset = 0;
    for (int j = 0; j < Objects[i].numMatFaces; j++) {
	...
	glPushMatrix();
      ...
      VertPosTexNorm::SetGLModes();
			...
			for (int j = 0; j < Objects[i].numMatFaces; j ++)
			{
				...
				Materials[Objects[i].MatFaces[j].MatIndex].tex->Bind();
          ...
          glDrawElements(GL_TRIANGLES, Objects[i].MatFaces[j].numSubFaces, GL_UNSIGNED_SHORT, (GLuint*)(Objects[i].MatFaces[j].iboOffset));
...
void Model_3DS::MapNameChunkProcessor(long length, long findex, int matindex)
{
  ...
  Materials[matindex].tex = Texture::LoadFromFile(fullname);
